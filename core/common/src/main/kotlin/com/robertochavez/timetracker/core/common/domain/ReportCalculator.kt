...
class ReportCalculator(
    ...
    fun daily(date: LocalDate, sessions: List<AwaySession>, intervals: List<ActivityInterval>, now: Instant): DailyReport {
    ...
    fun weekly(dateInWeek: LocalDate, sessions: List<AwaySession>, intervals: List<ActivityInterval>, now: Instant): PeriodReport {
    ...
    fun biweekly(dateInPeriod: LocalDate, sessions: List<AwaySession>, intervals: List<ActivityInterval>, now: Instant): PeriodReport {
    ...
    fun monthly(month: YearMonth, sessions: List<AwaySession>, intervals: List<ActivityInterval>, now: Instant): PeriodReport {
        val start = month.atDay(1)
        return period(DateRange(start, month.plusMonths(1).atDay(1)), sessions, intervals, now)
    }

    fun yearly(year: Int, sessions: List<AwaySession>, intervals: List<ActivityInterval>, now: Instant): PeriodReport {
        val start = LocalDate.of(year, 1, 1)
        return period(DateRange(start, start.plusYears(1)), sessions, intervals, now)
    }

    private fun period(range: DateRange, sessions: List<AwaySession>, intervals: List<ActivityInterval>, now: Instant): PeriodReport {
